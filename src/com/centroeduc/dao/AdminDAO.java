...
public class AdminDAO extends Conexion {
    ...
    public Administrador searchUser(String usuario, String contra) {
        Administrador admin = new Administrador();
        //JOptionPane.showMessageDialog(null, "usuario" + usuario + "contrase" + contra);
        try {
            this.Conectar();
            sql = "Select * from administrador where email=? and password=Md5(?)";
            ejecutar = this.getMiconexion().prepareStatement(sql);
            ejecutar.setString(1, usuario);
            ejecutar.setString(2, contra);
            valores = ejecutar.executeQuery();
            if (valores.next()) {
                admin.setCodigo(valores.getString("cod_admin"));
                admin.setNombre(valores.getString("nombre"));
                admin.setApellido(valores.getString("apellido"));
                admin.setDireccion(valores.getString("direccion"));
                admin.setEmail(valores.getString("email"));
                admin.setTelCasa(valores.getInt("tel_casa"));
                admin.setTelMovil(valores.getInt("tel_movil"));
                admin.setFechanac(valores.getString("fechanac"));
                admin.setCui(valores.getLong("cui"));
                admin.setPass(valores.getString("password"));
                admin.setEstado(valores.getInt("estado"));
            } 
